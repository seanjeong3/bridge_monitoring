...
app.get('/sensordata/:id', function (request, response) {
	...
	cassClient.execute(cassQueryStmt, cassQueryVal, function(err, result) {
		if (err) {
			...
			console.log("success: get /sensor_data_trb");
...
app.post('/sensordata', function (request, response) {   
	...
	// Inser parsed queries into DB
	cassClient.batch(queries, { prepare: true }, function (err) {
   		if (err) {
            response.status(400).end();
		} else {
			console.log("success: post /sensor_data_trb");
 ...
 * --------------------
 */ 
app.get('/sensordata_oneday/:id', function (request, response) {
...
/* 
 * --------------------
 * GET: /sensordata_oneweek
 * --------------------
 */ 
app.get('/sensordata_oneweek/:id', function (request, response) {
	if (isAuthRequired && request.session.user_id === undefined) {
        response.status(401).end();
        return;
    }
